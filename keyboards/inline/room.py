...
from states.room_states import RoomQuizStates, GroupQuizStates
from utils.logging_utils import logger

room_service = RoomService(db, bot)
...
async def send_next_question(message: types.Message, state: FSMContext):
    try:
        ...
        if current_index < len(questions) and not data.get('quiz_ended'):
            question = questions[current_index]
            await state.update_data(
                true_answer=question['question'],
                current_index=current_index + 1,
                answered=False,
                current_task_id=random.randint(1, 1_000_000)
            )

            await GroupQuizStates.waiting_answer.set()

            question_text = (f"{current_index + 1}/{len(questions)}\n"
                             f"<b>Savol:</b> {question['answer_a']}")
            await message.answer(question_text, parse_mode='HTML')

            timeout_task = asyncio.create_task(
                check_answer_timeout(message, state, data['current_task_id'])
            )
            await state.update_data(timeout_task_id=id(timeout_task))

        elif not data.get('quiz_ended'):
...
@dp.callback_query_handler(lambda c: c.data.startswith('quiz_battle_'),
                           state=RoomQuizStates.selecting_battle)
async def handle_battle_selection(callback: types.CallbackQuery, state: FSMContext):
    try:
        quiz_id = int(callback.data.split('_')[-1])
        await state.update_data(quiz_id=quiz_id)
        await callback.message.answer("Savollar sonini kiriting:")
        await RoomQuizStates.quiz_number.set()
    except Exception as e:
        logger.error(f"Battle selection error: {e}")
        await callback.message.answer("Xatolik yuz berdi")


@dp.message_handler(state=GroupQuizStates.waiting_answer)
async def handle_answer(message: types.Message, state: FSMContext):
    try:
        data = await state.get_data()
        is_correct = await question_service.check_answer(
            message.text,
            data['true_answer']
        )
        ...
        await state.update_data(answers_list=answers, answered=True)

        result_text = "Javobingiz to'g'ri!" if is_correct else f"Noto'g'ri! To'g'ri javob: {data['true_answer']}"
...
async def handle_rating(callback: types.CallbackQuery):
    try:
        ...
        results = await quiz_service.get_quiz_results(unique_id)
        formatted_results = await quiz_service.format_rating(results)
        await callback.message.answer(formatted_results)
    except Exception as e:
        logger.error(f"Rating handling error: {e}")
        await callback.message.answer("Xatolik yuz berdi")


async def check_answer_timeout(message: types.Message, state: FSMContext, task_id: int):
    await asyncio.sleep(question_service.TIMEOUT_DURATION)
    data = await state.get_data()

    if data.get('answered') or data.get('current_task_id') != task_id or data.get('quiz_ended'):
        return
...
async def end_quiz(message: types.Message, state: FSMContext):
    ...
    formatted_result = await question_service.format_quiz_result(answers, name)
    await room_service.save_quiz_result(
